	       ...
	       int& k1, int& k2)
  {
	...
	if (lambda.k() <= basis.DeltaLmax())
	...
	else
	  {
		...
		k1 = basis.DeltaRmin(lambda.j()) + ell1<d>();
		k2 = 1<<lambda.j();
	      }
	    else
	      {
	...
	const SparseMatrix<double>& Mj1_t = basis.Mj1_transposed();
	...
	int dummy;
	support(basis, typename DKUBasis<d,dT>::Index(lambda.j()+1, 0, kleft, &basis), k1, dummy);
