...
//     discrete_poisson.set_Ainv(1.0/0.146);
  ...
  std::ofstream ofs5("./sd_results22/approx_sol_steep_1D_out.m");
  matlab_output(ofs5,U);
  ofs5.close();

  std::ofstream ofs6("sd_results22/error_steep_1D_out.m");
  ...
  for (int i = 0; i < frame.n_p(); i++) {
    ...
    char filename3[128];
    sprintf(filename3, "%s%d%s%d%s%d%s", "sd_results22/approx1Dsteep_local_on_patch_" , i , "_d" , d ,  "_dT", dT, ".m");
  ...
  std::ofstream ofs7("./sd_results22/indices_patch_0.m");
  WaveletTL::plot_indices<Basis1D>(frame.bases()[0]->bases()[0], indices[0], JMAX, ofs7, "jet", true, -16);

  std::ofstream ofs8("./sd_results22/indices_patch_1.m");
