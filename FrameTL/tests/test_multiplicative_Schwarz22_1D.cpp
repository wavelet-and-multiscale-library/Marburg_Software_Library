...
//   // #######################################################
  ...
  char filename1[128];
  sprintf(filename1, "%s%d%s%d%s", "./ms_results22/approx1D_global_d", d, "_dT", dT, ".m");
  ...
  char filename2[128];
  sprintf(filename2, "%s%d%s%d%s", "./ms_results22/error1D_global_d", d, "_dT", dT, ".m");
  ...
  for (int i = 0; i < frame.n_p(); i++) {
    ...
    char filename3[128];
    sprintf(filename3, "%s%d%s%d%s%d%s", "./ms_results22/approx1D_local_on_patch_" , i , "_d" , d ,  "_dT", dT, ".m");
  ...
  std::ofstream ofs7("./ms_results22/indices_patch_0.m");
  WaveletTL::plot_indices<Basis1D>(frame.bases()[0]->bases()[0], indices[0], JMAX, ofs7, "jet", true, -16);

  std::ofstream ofs8("./ms_results22/indices_patch_1.m");
