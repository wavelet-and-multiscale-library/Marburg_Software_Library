...
#ifndef _WAVELETTL_CACHED_QUARKLET_DOMAIN_PROBLEM_H
#define	_WAVELETTL_CACHED_QUARKLET_DOMAIN_PROBLEM_H
...
namespace WaveletTL
{
     ...
     * frame is assumed to be of ldomain tensor type as modeled by ldomain_frame.h.
     ...
     *
     * The template class CachedQuarkletLDomainProblem implements the minimal signature to be
     * used within the APPLY routine.
     */
    template <class PROBLEM>
    class CachedQuarkletDomainProblem
#ifdef DYADIC
    ...
    public:
         ...
         * (if zero, CachedQuarkletDomainProblem will compute the estimates)
         */
        CachedQuarkletDomainProblem(PROBLEM* P,
...
//            return sqrt(a(lambda,lambda));//this needs to be changed to dyadic preconditioner
         ...
         */
        void set_normA(double norm) { normA = norm; }

        /*
         * set the spectral norm ||A^{-1}||
         */
        void set_normAinv(double norm) { normAinv = norm; }
...
#include "cached_quarklet_domain_problem.cpp"

#endif	/* _WAVELETTL__CACHED_QUARKLET_DOMAINPROBLEM_H */
