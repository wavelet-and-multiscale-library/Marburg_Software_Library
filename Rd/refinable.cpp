...
namespace WaveletTL
{
  ...
  RefinableFunction<MASK>::evaluate(const int resolution) const
  {
  ...
  template <class MASK>
  InfiniteVector<double, int>
  RefinableFunction<MASK>::evaluate(const int mu, const int resolution) const
  {
    assert(resolution >= 0);
    ...
    int suppleft  = MASK::abegin();
    int suppright = MASK::aend();
	...
	for (int n = 0; n < suppright-suppleft-1; n++) {
	  A(m, n) = MASK::a(2*(suppleft+1+m)-(suppleft+1+n));
	}
	A(m, m) -= ldexp(1.0, -mu);
	// b[m] = 0;
      }
	...
	for (int i = (1<<(newres-1))*suppleft; i <= (1<<(newres-1))*suppright-1; i++) {
	  int m = 2*i+1;
	  for (int k = MASK::abegin(); k <= MASK::aend(); k++)
	    r[m] += MASK::a(k) * coarse.get_coefficient(m - (1<<(newres-1))*k);
	}
      }   
    }
  ...
  template <class MASK>
  SampledMapping<1>
  RefinableFunction<MASK>::evaluate(const int mu,
				    const int j,
				    const int k,
				    const int a,
				    const int b,
				    const int resolution) const
  {
    ...
    for (typename InfiniteVector<double, int>::const_iterator it(help.begin());
	 it != help.end(); ++it) {
      v.set_coefficient(it.index()+(1<<(resolution-j))*k, *it * factor);
    }
