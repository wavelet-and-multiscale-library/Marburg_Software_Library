// implementation for slitdomain_frame_equation.h
...
//    SlitDomainFrameEquation<IFRAME, SLITDOMAINFRAME>::SlitDomainFrameEquation(const EllipticBVP<2>* bvp,
...
//    }
    ...
    template <class IFRAME, class SLITDOMAINFRAME>
    SlitDomainFrameEquation<IFRAME, SLITDOMAINFRAME>::SlitDomainFrameEquation(const EllipticBVP<2>* bvp, const Frame* frame,
					    const bool precompute_rhs)
...
//            frame_->set_jpmax(multi_degree(frame_->j0()),0);
    ...
    template <class IFRAME, class SLITDOMAINFRAME>
    SlitDomainFrameEquation<IFRAME, SLITDOMAINFRAME>::SlitDomainFrameEquation(const SlitDomainFrameEquation& eq)
...
//            frame_->set_jpmax(multi_degree(frame_->j0()),0);
    ...
    template <class IFRAME, class SLITDOMAINFRAME>
    void
    SlitDomainFrameEquation<IFRAME, SLITDOMAINFRAME>::compute_rhs()
    {
        cout << "SlitDomainFrameEquation(): precompute right-hand side..." << endl;
...
//        cout<<"number of threads: "<<omp_get_num_threads()<<endl;
    ...
    inline
    double
    SlitDomainFrameEquation<IFRAME, SLITDOMAINFRAME>::D(const Index& lambda) const
    {
...
//        return 1;
    ...
    inline
    double
    SlitDomainFrameEquation<IFRAME, SLITDOMAINFRAME>::a(const Index& lambda,
    ...
    template <class IFRAME, class SLITDOMAINFRAME>
    double
    SlitDomainFrameEquation<IFRAME, SLITDOMAINFRAME>::a(const Index& la,
			     const Index& nu,
...
//            
    ...
    template <class IFRAME, class SLITDOMAINFRAME>
    double
    SlitDomainFrameEquation<IFRAME, SLITDOMAINFRAME>::f(const Index& lambda) const
    {
        // f(v) = \int_0^1 g(t)v(t) dt
//        cout << "SlitDomainEquation::f() called with lambda=" << lambda << endl;
        double r = 0;
...
//                cout << "current support: ["<<a[0]<<","<<b[0]<<"]x["<<a[1]<<","<<b[1]<<"]"<<endl;
                ...
                //special treat for patch 1 and 2 if the quarklet is extended:    
                ...
                if(patch==2){
                    if(mother_patch==3){ //lambda has been extended from north so south, reflect gauss points in y-direction
                        for(int i=0;i<(int)lambda_gauss_points[1].size();i++){
                            lambda_gauss_points[1][i]=1-lambda_gauss_points[1][i];
                        }
                        evaluate(*(frame_->frame1d_11()), 0,        //different 1d-frames for x- and y-direction due to boundary conditions
                    ...
                    if(mother_patch==2){                //psi_lambda has not been extended, evaluate with mixed bc
                        evaluate(*(frame_->frame1d_01()), 0,        
                                            lambda.p()[0],
                                            lambda.j()[0],
                                            lambda.e()[0],
...
//                }
                ...
                if(patch==3){   // compute the point values of the integrand (where we use that it is a tensor product)
                    evaluate(*(frame_->frame1d_11()), 0,        //different 1d-frames for x- and y-direction due to boundary conditions
                                            lambda.p()[0],
                                            lambda.j()[0],
                                            lambda.e()[0],
        ...
        //cout << "SlitDomainEquation::f() result is r=" << r << endl;
	return r;  
    }
    
    template <class IFRAME, class SLITDOMAINFRAME>
    void
    SlitDomainFrameEquation<IFRAME, SLITDOMAINFRAME>::RHS(const double eta,
    ...
    template <class IFRAME, class SLITDOMAINFRAME>
    double
    SlitDomainFrameEquation<IFRAME, SLITDOMAINFRAME>::norm_A() const
    {
        if (normA == 0.0) {
...
//        cout << "Eigenwerte: " << evals << endl;
    ...
    template <class IFRAME, class SLITDOMAINFRAME>
    double
    SlitDomainFrameEquation<IFRAME, SLITDOMAINFRAME>::norm_Ainv() const
    {
        if (normAinv == 0.0) {
        typedef typename Frame::Index Index;
        typedef typename Index::polynomial_type polynomial_type;
...
//            cout << *it << endl;
    ...
    template <class IFRAME, class SLITDOMAINFRAME>
    double
    SlitDomainFrameEquation<IFRAME, SLITDOMAINFRAME>::s_star() const
    {
        // notation from [St04a]
    ...
    template <class IFRAME, class SLITDOMAINFRAME>
        void
        SlitDomainFrameEquation<IFRAME, SLITDOMAINFRAME>::compute_diagonal()
        {
          cout << " SlitDomainFrameEquation(): precompute diagonal of stiffness matrix..." << endl;
...
//      #endif
    ...
    template <class IFRAME, class SLITDOMAINFRAME>
    double
    SlitDomainFrameEquation<IFRAME, SLITDOMAINFRAME>:: integrate(const IndexQ1D<IFRAME>& la,
...
//            cout << "Mainpatch: " << main_patch << endl;
            ...
            mirrored_la=((lambda->patch() == 4 && dir == 1) || (lambda->patch() == 5 && dir == 0) || (lambda->patch() == 6 && dir == 1)) ? 1 : 0;
            mirrored_mu=((mu->patch() == 4 && dir == 1) || (mu->patch() == 5 && dir == 0) || (mu->patch() == 6 && dir == 1)) ? 1 : 0;
