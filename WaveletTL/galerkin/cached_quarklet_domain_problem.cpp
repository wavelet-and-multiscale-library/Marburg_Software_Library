// implementation for cached_quarklet_domain_problem.h

namespace WaveletTL
{
    template <class PROBLEM>
    CachedQuarkletDomainProblem<PROBLEM>::CachedQuarkletDomainProblem(PROBLEM* P,
    ...
    template <class PROBLEM>
    double
    CachedQuarkletDomainProblem<PROBLEM>::a(const Index& la,
...
#pragma omp parallel num_threads(NUM_THREADS) 
                ...
                {
#if PARALLEL_A==1
#pragma omp for private(entry, r) schedule(dynamic) nowait
...
//                  cout << "Ergenis: " << r << endl;
    ...
    template <class PROBLEM>
    void
    CachedQuarkletDomainProblem<PROBLEM>::add_ball(const Index& lambda,
				      Vector<double>& w,
				      const int radius,
				      const double factor,
				      const int maxlevel,
...
#pragma omp critical //(write_output)
    ...
    template <class PROBLEM>
    double
    CachedQuarkletDomainProblem<PROBLEM>::norm_A() const
    {
        if (normA == 0.0)
        {
            //const int offset = 2;
            ...
            cout << "CachedQuarkletDomainProblem()::norm_A() called..." << endl;
#endif
...
//            cout << "cached_domain_problem.norm_A :: last quarklet = " << (problem->frame().last_quarklet(multi_degree(problem->frame().j0())+offsetj,p)) 
...
//            double help;
 //           LanczosIteration(A_Lambda, 1e-6, help, normA, 200, iterations);
    ...
    template <class PROBLEM>
    void
    CachedQuarkletDomainProblem<PROBLEM>::normtest(unsigned int offsetj, unsigned int offsetp) const
    {
        //if (normA == 0.0)
...
//#endif
    ...
    template <class PROBLEM>
    double
    CachedQuarkletDomainProblem<PROBLEM>::norm_Ainv() const
    {
...
#if _WAVELETTL_CACHEDPROBLEM_VERBOSITY >= 1
            cout << "CachedQuarkletDomainProblem()::norm_Ainv() called..." << endl;
...
//            LanczosIteration(A_Lambda, 1e-6, help, normA, 200, iterations);
    ...
    template <class PROBLEM>
    void
    CachedQuarkletDomainProblem<PROBLEM>::set_f(const Function<PROBLEM::space_dimension>* fnew)
    {
        problem->set_f(fnew);
    }
