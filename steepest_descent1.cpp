...
public:
  ...
  void steepest_descent1_SOLVE(const PROBLEM& P,  const double epsilon,
			      InfiniteVector<double, typename PROBLEM::Index>& u_epsilon, int jmax)
  {
    ...
    */
    ...
    //APPLY_COARSE(P, w,delta*l2_norm(w), r, 0.00001, jmax, CDD1); 
    APPLY_COARSE(P, w,omega_i, r, 0.00001, jmax, CDD1);
    r*=-1;
    //  P.RHS(delta*l2_norm(w),b);
    P.RHS(omega_i,b);
    r+=b;

    while(loops<20000 & !exit){
      cout << "omega_i:" << omega_i << endl;
      loops++;
      cout << "Scheife:" << loops << endl;
      p=r;
      //     p*=-1;
      //     APPLY_COARSE(P, r ,delta*l2_norm(w), Ap , 0.00001, jmax, CDD1);
      APPLY_COARSE(P, r ,omega_i*eps, Ap , 0.00001, jmax, CDD1);
      t=(r*r)/(r*Ap);
      p*=t;
      w+=p;
   
      //   if(l2_norm(r) < eps){ exit = true; cout << "near enough";}
