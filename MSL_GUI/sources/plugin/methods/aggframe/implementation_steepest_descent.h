...
#ifndef STEEPEST_DESCENT_GUI_H
#define STEEPEST_DESCENT_GUI_H
...
void steepest_descent_GuiSolve(const PROBLEM& P,  const double epsilon,
                            Array1D<InfiniteVector<double, typename PROBLEM::Index> >& approximations,
                            int jmax, MathTL::AbstractConvergenceLogger& logger)
{
    ...
    try
    {
        ...
        for (unsigned int i = 1; i <= K; i++)
        {
            ...
            WaveletTL::RES(P, w, xi_i, delta, omega_i/((1+3.*mu)*a_inv), jmax,
                tilde_r, nu_i, niter, WaveletTL::CDD1);
            ...
            while ( nu_i > omega_i/((1+3.*mu)*a_inv))
            {
                ...
                WaveletTL::APPLY_COARSE(P, tilde_r, delta*l2_norm(tilde_r), z_i, 1.0e-6, jmax, WaveletTL::CDD1);
            ...
            // and log convergence data.
            // #####################################################################################

            logger.pauseClock();

            P.RHS(1.0e-6,f);
            WaveletTL::APPLY(P, w, 1.0e-6, Av, jmax, WaveletTL::CDD1);
            help = f-Av;
