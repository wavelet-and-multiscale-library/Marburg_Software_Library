...
{
    problemDescription_ = QString("Poisson boundary value problem on the L-domain %1 = (-1,1)²\\[0,1)² with homogeneous Dirichlet boundary conditions: "
                                  "Find u(x,y) with\n"
                                  "\n"
...
{
public:
    RHS_TestProblem1() : csrhs_(MathTL::Point<2>(0,0), 0.5, 1.5) { }
...
void ProblemType::setupExamples()
{
    ...
    std::array<QString, 1> rhs_problem2 = {QString("-%1s(x,y) with corner singularity s").arg(QChar(0x2206))};
    addExampleProblem("Corner Singularity", rhs_problem2, new MathTL::PoissonBVP<2>(new MathTL::CornerSingularityRHS(MathTL::Point<2>(0,0), 0.5, 1.5)));
}
