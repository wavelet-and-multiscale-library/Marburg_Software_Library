...
int main(int argc, char* argv[])
{
  ...
  SymmetricMatrix<bool> adj(PATCHES);
  Array1D<Chart<DIM,DIM>* > charts(PATCHES);
  //to specify primal boundary the conditions
  Array1D<FixedArray1D<int,2*DIM> > bc(PATCHES);
...
//   }
  ...
  if (rank==0) {
    ...
    char filename1[128];
#ifdef PARALLEL
    sprintf(filename1, "%s%d%s%d%s", "./as_results2D_33_3patch_par/approx2D_global_d", d, "_dT", dT, ".m");
#else
    sprintf(filename1, "%s%d%s%d%s", "./as_results2D_33_3patch/approx2D_global_d", d, "_dT", dT, ".m");
#endif
    ...
    char filename2[128];
#ifdef PARALLEL
    sprintf(filename2, "%s%d%s%d%s", "./as_results2D_33_3patch_par/error2D_global_d", d, "_dT", dT, ".m");
#else
    sprintf(filename2, "%s%d%s%d%s", "./as_results2D_33_3patch/error2D_global_d", d, "_dT", dT, ".m");
#endif
    ...
    for (int i = 0; i < frame.n_p(); i++) {
      ...
      char filename3[128];
#ifdef PARALLEL
      sprintf(filename3, "%s%d%s%d%s%d%s", "./as_results2D_33_3patch_par/approx2D_local_on_patch_" , i , "_d" , d ,  "_dT", dT, ".m");
#else
      sprintf(filename3, "%s%d%s%d%s%d%s", "./as_results2D_33_3patch/approx2D_local_on_patch_" , i , "_d" , d ,  "_dT", dT, ".m");
#endif
    ...
    for (int i = 0; i < frame.n_p(); i++) {
      ...
      char filename4[128];
#ifdef PARALLEL
      sprintf(filename4, "%s%d%s%d%s%d%s", "./as_results2D_33_3patch_par/coefficient_plot_2D_patch_" , i , "_d" , d ,  "_dT", dT, ".m");
#else
      sprintf(filename4, "%s%d%s%d%s%d%s", "./as_results2D_33_3patch/coefficient_plot_2D_patch_" , i , "_d" , d ,  "_dT", dT, ".m");
#endif
