...
#ifndef _WAVELETTL_PERIODIC_FRAME_SUPPORT_H
#define _WAVELETTL_PERIODIC_FRAME_SUPPORT_H
...
namespace WaveletTL
{
  template <class RFRAME> class PeriodicFrame;
  
  /*!
    For a given wavelet \psi_\lambda, compute all generators/wavelets
    \psi_\nu with level |\nu|=j, nu.p()=p such that the respective supports
  ...
  */
  template <class RFRAME>
  bool intersect_singular_support(const PeriodicFrame<RFRAME>& basis,
                                  const typename PeriodicFrame<RFRAME>::Index& lambda,
				  const typename PeriodicFrame<RFRAME>::Index& nu)
  {
  ...
  */
  template <class RFRAME>
  void support(const PeriodicFrame<RFRAME>& basis,
	       const typename PeriodicFrame<RFRAME>::Index& lambda,
	       int& k1, int& k2)
  {
...
#include <interval/periodic_frame_support.cpp>
