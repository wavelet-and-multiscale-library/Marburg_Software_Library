// implementation for recring_frame_gramian.h
...
//    RecRingFrameGramian<IFRAME, RECRINGFRAME>::RecRingFrameGramian(const EllipticBVP<2>* bvp,
...
//    }
    ...
    template <class IFRAME, class RECRINGFRAME>
    RecRingFrameGramian<IFRAME, RECRINGFRAME>::RecRingFrameGramian(const EllipticBVP<2>* bvp, const Frame* frame,
					    const bool precompute_rhs)
...
//            frame_->set_jpmax(multi_degree(frame_->j0()),0);
    ...
    template <class IFRAME, class RECRINGFRAME>
    RecRingFrameGramian<IFRAME, RECRINGFRAME>::RecRingFrameGramian(const RecRingFrameGramian& eq)
...
//            frame_->set_jpmax(multi_degree(frame_->j0()),0);
    ...
    template <class IFRAME, class RECRINGFRAME>
    void
    RecRingFrameGramian<IFRAME, RECRINGFRAME>::compute_rhs()
    {
        cout << "RecRingFrameGramian(): precompute right-hand side..." << endl;
...
//        cout<<"number of threads: "<<omp_get_num_threads()<<endl;
    ...
    inline
    double
    RecRingFrameGramian<IFRAME, RECRINGFRAME>::D(const Index& lambda) const
    {
...
//        return 1;
    ...
    inline
    double
    RecRingFrameGramian<IFRAME, RECRINGFRAME>::a(const Index& lambda,
    ...
    template <class IFRAME, class RECRINGFRAME>
    double
    RecRingFrameGramian<IFRAME, RECRINGFRAME>::a(const Index& la,
			     const Index& nu,
...
//            
    ...
    template <class IFRAME, class RECRINGFRAME>
    double
    RecRingFrameGramian<IFRAME, RECRINGFRAME>::f(const Index& lambda) const
    {
...
//        const bool extended=(lambda->patch()>2) ? 1 : 0;
 	...
 	for (int patch = 0; patch <= 7; patch++) {
 	    ...
 	    if (supp.xmin[patch] != -1) { // psi_mu is nontrivial on patch p
                ...
                if(patch==2 || patch==4){
                    ...
                    if(mother_patch==3){ //lambda has been extended from east to west or vice versa, reflect gauss points in x-direction
                        for(int i=0;i<(int)lambda_gauss_points[1].size();i++){
                ...
                if(patch==0 || patch==6){
                    ...
                    if(mother_patch==7){ //lambda has been extended from eat to west or vice versa, reflect gauss points in x-direction
                        for(int i=0;i<(int)lambda_gauss_points[1].size();i++){
                            lambda_gauss_points[0][i]=1-lambda_gauss_points[0][i];
                        }
                        evaluate(*(frame_->frame1d()), 0,        //different 1d-frames for x- and y-direction due to boundary conditions
    ...
    template <class IFRAME, class RECRINGFRAME>
    void
    RecRingFrameGramian<IFRAME, RECRINGFRAME>::RHS(const double eta,
    ...
    template <class IFRAME, class RECRINGFRAME>
    double
    RecRingFrameGramian<IFRAME, RECRINGFRAME>::norm_A() const
    {
        if (normA == 0.0) {
...
//        cout << normA << ", " << normAinv << endl;
    ...
    template <class IFRAME, class RECRINGFRAME>
    double
    RecRingFrameGramian<IFRAME, RECRINGFRAME>::norm_Ainv() const
    {
        if (normAinv == 0.0) {
        typedef typename Frame::Index Index;
        typedef typename Index::polynomial_type polynomial_type;
...
//            cout << *it << endl;
    ...
    template <class IFRAME, class RECRINGFRAME>
    double
    RecRingFrameGramian<IFRAME, RECRINGFRAME>::s_star() const
    {
        // notation from [St04a]
    ...
    template <class IFRAME, class RECRINGFRAME>
        void
        RecRingFrameGramian<IFRAME, RECRINGFRAME>::compute_diagonal()
        {
          cout << " RecRingFrameGramian(): precompute diagonal of stiffness matrix..." << endl;
...
//      #endif
    ...
    template <class IFRAME, class RECRINGFRAME>
    double
    RecRingFrameGramian<IFRAME, RECRINGFRAME>:: integrate(const IndexQ1D<IFRAME>& la,
...
//        cout << mu->number() << endl;
        ...
        if (lb == col.end() ||
            col.key_comp()(*mu, lb->first))
          {
            ...
            int main_patch;           //the patch we will perform our computations on
...
//                int e = std::max(lambda->e()[i], mu->e()[i]); //correct granularity
            ...
            //does not occur since we have no derivatives in the gramian
//            if(main_patch == 1){
