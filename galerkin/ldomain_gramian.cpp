...
namespace WaveletTL
{
   ...
   const Index& lambda2) const
  {
	...
	psi_lambda1_values; // point values of the integrands
	...
	if (supp.xmin[p] != -1) { // psi_lambda1 is nontrivial on patch p
	  ...
	  basis_.evaluate(lambda1, p, gauss_points0, gauss_points1, psi_lambda1_values);
	  ...
	  for (unsigned int i0 = 0, id = 0; i0 < gauss_points0.size(); i0++) {
	    ...
	    for (unsigned int i1 = 0; i1 < gauss_points1.size(); i1++, id++) {
	      help += gauss_weights1[i1] * psi_lambda1_values[id] * psi_lambda1_values[id];
	    }
	...
	for (int p = 0; p <= 2; p++) {
	  if (supp.xmin[p] != -1) { // psi_lambda1 and psi_lambda2 are nontrivial on patch p
	    ...
	    basis_.evaluate(lambda1, p, gauss_points0, gauss_points1, psi_lambda1_values);
	    basis_.evaluate(lambda2, p, gauss_points0, gauss_points1, psi_lambda2_values);	  
	    
	    // aggregate the integral shares
	    for (unsigned int i0 = 0, id = 0; i0 < gauss_points0.size(); i0++) {
	      double help = 0;
	      for (unsigned int i1 = 0; i1 < gauss_points1.size(); i1++, id++) {
		help += gauss_weights1[i1] * psi_lambda1_values[id] * psi_lambda2_values[id];
	      }
	      r += gauss_weights0[i0] * help;
	    }
	  }
