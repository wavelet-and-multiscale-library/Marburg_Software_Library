...
//       cout << "APPLY(): v binned looks as follows:" << endl;
...
// 	cout << v_binned[i].first << ", " << v_binned[i].second << endl;
      ...
      // i.e.
      //   ||\sum_{k=0}^\ell v_{[k]}||^2 >= ||v||^2 - eta^2/(4*||A||^2)
      // see [S, (3.9)]
      const double threshold = norm_v_sqr - eta*eta/(4*norm_A*norm_A);
...
// 	cout << "APPLY(): setup v_{[" << k << "]}" << endl;
	std::list<std::pair<Index, double> > vk;
	double vk_norm_sqr = 0;
	for (unsigned int n = 1; E < threshold && id < v.size() && n <= ldexp(1.0, k)-floor(ldexp(1.0, k-1)); n++, id++) {
	...
	if (E >= threshold || id >= v.size()) break; // in this case, ell=k
	k++;
      }
...
// 	  cout << "APPLY(): requesting column " << itk->first << " with J-k=" << J-k << endl;
	  P.add_column(itk->second, itk->first, J-k, w);
	}
      }
    }
  }  
