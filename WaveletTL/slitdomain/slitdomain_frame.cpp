// implementation for slitdomain_frame.h
...
namespace WaveletTL
{
  ...
  SlitDomainFrame<IFRAME>::SlitDomainFrame(const IntervalFrame* frame1d, const IntervalFrame* frame1d_11, 
...
//  template <class IFRAME>
//  SlitDomainFrame<IFRAME>::SlitDomainFrame(const IntervalFrame& frame1d)
...
//  }
  ...
  template <class IFRAME>
  const int
  SlitDomainFrame<IFRAME>::Deltasize(const int j) const {
    const unsigned int Deltaj = frame1d_->Deltasize(j);
    return 4*(Deltaj-2)*(Deltaj-2)+3*(Deltaj-2);
  }

//  template <class IBASIS>
//  const int
//  SlitDomainBasis<IBASIS>::Nabla01size(const int j) const {
//    return (3*basis1d().Deltasize(j)-5)*(1<<j);
//  }
//  
//  template <class IBASIS>
//  const int
//  SlitDomainBasis<IBASIS>::Nabla10size(const int j) const {
//    return (3*basis1d().Deltasize(j)-5)*(1<<j);
//  }
//  
//  template <class IBASIS>
//  const int
//  SlitDomainBasis<IBASIS>::Nabla11size(const int j) const {
//    return 3*(1<<(2*j));
//  }

  
  template <class IFRAME>
  typename SlitDomainFrame<IFRAME>::Index
  SlitDomainFrame<IFRAME>::first_generator(const level_type& j, const polynomial_type& p) const
  {
    assert(j >= j0_);
...
//        return Index(p,j, e, 0, k, number, this);
  ...
  template <class IFRAME>
  typename SlitDomainFrame<IFRAME>::Index
  SlitDomainFrame<IFRAME>::last_generator(const level_type& j, const polynomial_type& p) const
  {
    assert(j >= j0_);
...
//    typename Index::translation_type k(frame1d_->DeltaLmin(), frame1d_->DeltaRmax(j[1])-1);
    ...
    typename Index::translation_type k(frame1d_->DeltaRmax(j[0])-1, frame1d_->DeltaLmin());
    
    return Index(p,j, e, 6, k, p.number()* Nablasize_+Deltasize(j[0])-1, this);
  }

  template <class IFRAME>
  typename SlitDomainFrame<IFRAME>::Index
  SlitDomainFrame<IFRAME>::first_quarklet(const level_type& j, const polynomial_type& p, const int& number) const
  {
    assert(j >= j0());
...
//  template <class IFRAME>
//  typename SlitDomainFrame<IFRAME>::Index
//  SlitDomainFrame<IFRAME>::first_quarklet(const level_type& j, const type_type& e, const polynomial_type& p) const
//  {
//    assert(j >= j0());
//    
...
//  }
  ...
  template <class IFRAME>
  typename SlitDomainFrame<IFRAME>::Index
  SlitDomainFrame<IFRAME>::last_quarklet(const level_type& j, const polynomial_type& p, const int& number) const
  {
    assert(j >= j0());
...
//    typename Index::translation_type k(0, frame1d_->Nablamax(j[1]));
    ...
    typename Index::translation_type k( frame1d_->Nablamax(j[0]),0);
//    cout << "in last_quarklet level_type" << endl;
    if (number==-1)
    {
        ...
        return Index(p,j, e, 6, k, altnumber, this);
    }
    else
    
       return Index(p,j, e, 6, k, number, this);
  }
  
  template <class IFRAME>
  typename SlitDomainFrame<IFRAME>::Index
  SlitDomainFrame<IFRAME>::last_quarklet(const int& levelsum, const polynomial_type& p, const int& number) const
  {
...
//        cout << "number" << endl;
  ...
  template <class IFRAME>
  void
  SlitDomainFrame<IFRAME>::support(const int& lambda_num, Support& supp) const
  {
     ...
     if (precomputed_supports_){
         ...
         cout << "SlitDomainFrame<IFRAME>::support(): precompute supports first!" << endl;
         abort();      
     }
  }
  ...
  template <class IFRAME>
  void
  SlitDomainFrame<IFRAME>::support(const Index& lambda, Support& supp) const
  {
...
//	typedef typename SupportCache::value_type value_type;
     ...
     if (precomputed_supports_){
	  ...
	  case 0:
	    ...
	    supp.xmin[1] = supp.xmin[2] = supp.xmin[3] = -1;
	  ...
	  case 1:
	    ...
	    supp.xmin[0] = supp.xmin[2] = supp.xmin[3] = -1;
	  ...
	  case 2:
	    ...
	    supp.xmin[0] = supp.xmin[1] = supp.xmin[3] = -1;
	    
	    break;
          case 3:
	    // psi_lambda completely lives on patch 3
	    ...
	    supp.xmin[0] = supp.xmin[1] = supp.xmin[2] = -1;
	  ...
	  case 5:
	    ...
	    supp.xmin[0] = supp.xmin[3] = -1;
	    
	    break;
          case 6:
	    // psi_lambda lives on patches 3 and 2
...
//      }  
  ...
  SlitDomainFrame<IFRAME>::evaluate
  (const typename SlitDomainFrame<IFRAME>::Index& lambda,
   const int resolution) const
  {
    Array1D<SampledMapping<2> > r(4);
    ...
    typename Index::type_type zero;
 	...
 	r[2] = SampledMapping<2>(Point<2>( 0,-1), Point<2>(1,0), values);
        r[3] = SampledMapping<2>(Point<2>(0,0), Point<2>(1,1), values);
 	break;
      case 1:
 	// psi_lambda completely lives on patch 1
 	values[0] = frame1d_11_->evaluate(typename IFRAME::Index(lambda.p()[0],
...
// 	for (int i = 0; i <= 1<<resolution; i++) values[1][i] *= M_SQRT1_2;
   ...
   const int resolution) const
  {
    ...
    Array1D<SampledMapping<2> > result(4);
    ...
    result[3] = SampledMapping<2>(Point<2>(0,0), Point<2>(1,1), values);
  ...
  template <class IFRAME>
  void
  SlitDomainFrame<IFRAME>::setup_full_collection()
  {
    if (jmax_ == -1 || jmax_ < (int) multi_degree(j0())) {
      cout << "SlitDomainFrame<IFRAME>::setup_full_collection(): specify a maximal level of resolution first!" << endl;
      abort();
    }   
