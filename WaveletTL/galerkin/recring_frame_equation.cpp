// implementation for recring_frame_equation.h
...
//    RecRingFrameEquation<IFRAME, RECRINGFRAME>::RecRingFrameEquation(const EllipticBVP<2>* bvp,
...
//    }
    ...
    template <class IFRAME, class RECRINGFRAME>
    RecRingFrameEquation<IFRAME, RECRINGFRAME>::RecRingFrameEquation(const EllipticBVP<2>* bvp, const Frame* frame,
					    const bool precompute_rhs)
...
//            frame_->set_jpmax(multi_degree(frame_->j0()),0);
    ...
    template <class IFRAME, class RECRINGFRAME>
    RecRingFrameEquation<IFRAME, RECRINGFRAME>::RecRingFrameEquation(const RecRingFrameEquation& eq)
...
//            frame_->set_jpmax(multi_degree(frame_->j0()),0);
    ...
    template <class IFRAME, class RECRINGFRAME>
    void
    RecRingFrameEquation<IFRAME, RECRINGFRAME>::compute_rhs()
    {
        cout << "RecRingFrameEquation(): precompute right-hand side..." << endl;
...
//            cout << it.index() <<", " << *it << endl;
    ...
    inline
    double
    RecRingFrameEquation<IFRAME, RECRINGFRAME>::D(const Index& lambda) const
    {
...
//        return 1;
    ...
    inline
    double
    RecRingFrameEquation<IFRAME, RECRINGFRAME>::a(const Index& lambda,
    ...
    template <class IFRAME, class RECRINGFRAME>
    double
    RecRingFrameEquation<IFRAME, RECRINGFRAME>::a(const Index& la,
			     const Index& nu,
...
//            
    ...
    template <class IFRAME, class RECRINGFRAME>
    double
    RecRingFrameEquation<IFRAME, RECRINGFRAME>::f(const Index& lambda) const
    {
        // f(v) = \int_0^1 g(t)v(t) dt
//        cout << "RecRingEquation::f() called with lambda=" << lambda << endl;
        double r = 0;
...
//        cout << "patch 3: [" << supp.xmin[3]  <<" , " <<supp.xmax[3] <<"]x["<<supp.ymin[3] <<" , "<<supp.ymax[3] <<"]"<<  endl; 
        
        //determine on which patch lambda lies

//        const bool extended=(lambda->patch()>3) ? 1 : 0;
...
//                cout << "current support: ["<<a[0]<<","<<b[0]<<"]x["<<a[1]<<","<<b[1]<<"]"<<endl;
                ...
                if(patch==1 || patch==5){   // compute the point values of the integrand (where we use that it is a tensor product)
//                    cout << "hier1"  << endl;
...
//                    cout << v_values[1] << endl;
                ...
                if(patch==3 || patch==7){   // compute the point values of the integrand (where we use that it is a tensor product)
                    evaluate(*(frame_->frame1d()), 0,        //different 1d-frames for x- and y-direction due to boundary conditions
                                            lambda.p()[0],
                                            lambda.j()[0],
                                            lambda.e()[0],
                                            lambda.k()[0],
                ...
                if(patch==2 || patch==4){
                    if(mother_patch==1 || mother_patch==5){ //lambda has been extended from north so south, reflect gauss points in y-direction
//                        cout << "hier2"  << endl;
...
//                        cout << v_values[1] << endl;
                    ...
                    else if(mother_patch==3){ //lambda has been extended from east to west or vice versa, reflect gauss points in x-direction
                    ...
                    else{                //psi_lambda has not been extended, evaluate with zero bc
                        evaluate(*(frame_->frame1d_11()), 0,        
                                            lambda.p()[0],
                                            lambda.j()[0],
                                            lambda.e()[0],
                ...
                if(patch==0 || patch==6){
                    if(mother_patch==1 || mother_patch==5){ //lambda has been extended from south to north, reflect gauss points in y-direction
                        for(int i=0;i<(int)lambda_gauss_points[1].size();i++){
                            lambda_gauss_points[1][i]=1-lambda_gauss_points[1][i];
                        }
                        evaluate(*(frame_->frame1d_11()), 0,        //different 1d-frames for x- and y-direction due to boundary conditions
                    ...
                    else if(mother_patch==7){ //lambda has been extended from eat to west or vice versa, reflect gauss points in x-direction
                        for(int i=0;i<(int)lambda_gauss_points[0].size();i++){
                            lambda_gauss_points[0][i]=1-lambda_gauss_points[0][i];
                        }
                        evaluate(*(frame_->frame1d()), 0,        //different 1d-frames for x- and y-direction due to boundary conditions
...
//                    if (lambda.number()==2703)
//                    cout << "f(" << x[0] << ", " << x[1] << "):" << share << endl;
                    for (int i = 0; i < space_dimension; i++){
//                        if (lambda.number()==2703){
//                            cout << "Gauss: " << gauss_weights[i][index[i]] << endl;
//                            cout << "Index: " << v_values[i][index[i]] << endl;
//                        }
...
//                        cout << "Summe: " << r << endl;
        ...
        //cout << "RecRingEquation::f() result is r=" << r << endl;
	return r;  
    }
    
    template <class IFRAME, class RECRINGFRAME>
    void
    RecRingFrameEquation<IFRAME, RECRINGFRAME>::RHS(const double eta,
    ...
    template <class IFRAME, class RECRINGFRAME>
    double
    RecRingFrameEquation<IFRAME, RECRINGFRAME>::norm_A() const
    {
        if (normA == 0.0) {
...
//        cout << "Eigenwerte: " << evals << endl;
    ...
    template <class IFRAME, class RECRINGFRAME>
    double
    RecRingFrameEquation<IFRAME, RECRINGFRAME>::norm_Ainv() const
    {
        if (normAinv == 0.0) {
        typedef typename Frame::Index Index;
        typedef typename Index::polynomial_type polynomial_type;
...
//            cout << *it << endl;
    ...
    template <class IFRAME, class RECRINGFRAME>
    double
    RecRingFrameEquation<IFRAME, RECRINGFRAME>::s_star() const
    {
        // notation from [St04a]
    ...
    template <class IFRAME, class RECRINGFRAME>
        void
        RecRingFrameEquation<IFRAME, RECRINGFRAME>::compute_diagonal()
        {
          cout << " RecRingFrameEquation(): precompute diagonal of stiffness matrix..." << endl;
...
//      #endif
    ...
    template <class IFRAME, class RECRINGFRAME>
    double
    RecRingFrameEquation<IFRAME, RECRINGFRAME>:: integrate(const IndexQ1D<IFRAME>& la,
                                                            ...
                                                            typename Frame::Support supp) const
      {
        ...
        double res=0.;
//        cout<<"called integrate, dir="<<dir<<endl;
...
//        cout << mu->number() << endl;
        ...
        if (lb == col.end() ||
            col.key_comp()(*mu, lb->first))
          {
            ...
            //first look if the quarklets have support on an even patch
            for(int patch = 0; patch<=7; patch+=2){
                if(supp.xmin[patch]!=-1){
                    main_patch=patch;
                    break;
                }
            }
            //if both quarklets have support on an odd patch, than this is the main_patch
            for(int patch = 1; patch<=7; patch+=2){
                if(supp.xmin[patch]!=-1){
                    main_patch=patch;
                    break;
                }
...
//            cout << "Mainpatch: " << main_patch << endl;
            ...
            mirrored_la=(((lambda->patch() == 8 || lambda->patch() == 9 || lambda->patch() == 12 || lambda->patch() == 13 ) && dir == 1)
                        || ((lambda->patch() == 10 || lambda->patch() == 11 || lambda->patch() == 14 || lambda->patch() == 15) && dir == 0)) ? 1 : 0;
            mirrored_mu=(((mu->patch() == 8 || mu->patch() == 9 || mu->patch() == 12 || mu->patch() == 13 ) && dir == 1)
                        || ((mu->patch() == 10 || mu->patch() == 11 || mu->patch() == 14 || mu->patch() == 15) && dir == 0)) ? 1 : 0;
//            cout<<mirrored_la<<endl;
//            cout<<mirrored_mu<<endl;
...
//            cout<<lambda_gauss_points<<endl;
//            cout<<mu_gauss_points<<endl;
...
//            if(main_patch == 2){        //new: different treatment for reflections
//                if(mirrored_la && lambda->patch()==6){
//                    for(int i=0;i<(int)lambda_gauss_points.size();i++){
//                             lambda_gauss_points[i]=1-lambda_gauss_points[i];
//                    }
//                }
//                if(mirrored_mu && mu->patch()==6){
//                    for(int i=0;i<(int)mu_gauss_points.size();i++){
//                             mu_gauss_points[i]=1-mu_gauss_points[i];
//                    }
//                }
...
//            if(main_patch == 2){
//                if(mirrored_la && lambda->patch()==6 && lambda->derivative() == 1){
//                    for(int i=0;i<(int)lambda_values.size();i++){
//                             lambda_values[i]=-lambda_values[i];
//                    }
//                }
//                if(mirrored_mu && mu->patch()==6 && mu->derivative() == 1){
//                    for(int i=0;i<(int)mu_values.size();i++){
//                             mu_values[i]=-mu_values[i];
//                    }
//                }
...
//            cout<<"res="<<res<<endl;
            ...
            //into the direction of integration; this works in the rectangular ring case, since functions 
            // that are mirrored to the left handside have trivial intersecting support with functions that are mirrored 
