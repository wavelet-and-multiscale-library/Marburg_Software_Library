// implementation for cached_lproblem.h

namespace WaveletTL
{
    template <class PROBLEM>
    CachedLProblem<PROBLEM>::CachedLProblem(PROBLEM* P,
    ...
    template <class PROBLEM>
    double
    CachedLProblem<PROBLEM>::a(const Index& lambda,
...
//TODO PERFORMANCE: store the numbrs of all levels up to jmax, do not compute anything here:
    ...
    template <class PROBLEM>
    void
    CachedLProblem<PROBLEM>::add_ball(const Index& lambda,
                                      ...
                                      const bool precond) const
    {
        ...
        {
            int j0 = this->basis().j0();
            Index mu;
            for (int level = max (j0, lambda.j()-radius); level  < min(lambda.j()+radius,maxlevel)+1;level++)
            {
                ...
                // using the first generator on level j0 would mean no difference, since block 0 contains wavelets and generators
                a(mu,lambda);
                // add the level
                ...
                for (typename Block::const_iterator it(block.begin()), itend(block.end()); it != itend; ++it)
                {
                    ...
                    w[it->first]=w[it->first]+factor*(it->second)/( precond? (d1*D(this->basis().get_wavelet(it->first))):1.0);
        ...
        else if (space_dimension == 2)
        {
            ...
            for (int offset = -std::min(dist2j0,radius); offset < std::min(dist2maxlevel,radius)+1; offset++)
            {
                ...
                xstart = lambda.j()-radius+ceil((radius+offset)/2.0); //x coordinate of the first level, ignoring restrictions by j0[0]
                xend = lambda.j()+floor((radius+offset)/2.0); // same for the last
                ystart = lambda.j()+floor((radius+offset)/2.0); // and for the second dimension
                // the first level in the ball is denoted with steps=0.
                // The first level on the current levelline in the ball may have steps >0.
                for (int steps = max(0,j0-xstart); steps <= min(xend-xstart,ystart-j0) ; steps++ )
                {
                    currentlevel= xstart+steps;
                    currentlevel= ystart-steps;
                    mu = this->basis().first_wavelet(currentlevel);

                    a(mu,lambda); // computes & stores the block in the cache
                    cout << mu << endl;
                    blocknumber = currentlevel-j0 + ((lambdaline-lowestline+offset)*(lambdaline-lowestline+offset+1))/2;
    ...
    template <class PROBLEM>
    void
    CachedLProblem<PROBLEM>::add_level_recurse(const Index& lambda,
                                               Vector<double>& w,
                                               const int radius,
                                               const double factor,
...
*/ // comment for the levelwindow code
    ...
    template <class PROBLEM>
    double
    CachedLProblem<PROBLEM>::norm_A() const
    {
        if (normA == 0.0)
        {
            //const int offset = 2;
            ...
            for (Index lambda ( problem->basis().first_generator() ), itend(problem->basis().last_wavelet(problem->basis().j0()+offset));; ++lambda)
            {
...
//#endif
    ...
    template <class PROBLEM>
    void
    CachedLProblem<PROBLEM>::normtest(unsigned int offset) const
    {
        //if (normA == 0.0)
        {
#if _WAVELETTL_CACHEDPROBLEM_VERBOSITY >= 1
...
//#endif
    ...
    template <class PROBLEM>
    double
    CachedLProblem<PROBLEM>::norm_Ainv() const
    {
    ...
    template <class PROBLEM>
    void
    CachedLProblem<PROBLEM>::set_f(const Function<PROBLEM::space_dimension>* fnew)
    {
        problem->set_f(fnew);
    }
    ...
    template <class PROBLEM>
    void
    CachedLProblem<PROBLEM>::apply(const std::set<int>& window, const Vector<double>& x,
...
//        typedef typename Index::type_type generator_type;
    ...
    // TODO PERFORMANCE: col_it zeigt jetzt auf die gerade eingefügte Column, durch das col_it++ am Ende der for Schleife also wieder auf die selbe Column ... ineffizient.
            ...
            else if (col_it->first == *win_it_col)
            {
                ...
                unsigned int k = 0;
                int row_key, first_level(basis().j0());
    ...
    // TODO Performance: replace the following lines with a function call to a nice mapping: Wavelet_number -> Level_number
                    ...
                    //}
                    row_key=row_key-first_level;
    // TODO PERFORMANCE: store the numbers of all levels up to jmax, do not compute anything here:
                    rowind_blocknumber = row_key;
    ...
    // TODO PERFORMANCE: as before a mapping IndexNumber->BlockNumber could speed up things
                        // this code is also executed in another branch of the if then else statement. apply any code changes also there
                        {
                            ...
                            while (true)
                            {
                                ...
                                //}
                                row_key_temp=row_key_temp-first_level;
                                rowind_blocknumber_temp = row_key_temp;
...
// TODO PERFORMANCE: "Shift this case upwards"
    template <class PROBLEM>
    bool
    CachedLProblem<PROBLEM>::CG(const std::set<int>& window, const Vector<double> &b, Vector<double> &xk,
