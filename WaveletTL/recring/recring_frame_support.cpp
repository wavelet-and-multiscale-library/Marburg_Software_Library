...
// implementation for recring_frame_support.h
...
namespace WaveletTL
{
  ...
  bool intersect_supports(const RecRingFrame<IFRAME>& frame,
        ...
        typedef typename RecRingFrame<IFRAME>::Support Support;
...
//        if (supp_la->xmin[0] == -1 && supp_la->xmin[1] == -1 && supp_la->xmin[2] == -1 && supp_mu->xmin[3] == -1) return false;
  ...
  bool intersect_supports(const RecRingFrame<IFRAME>& frame,
			  const typename RecRingFrame<IFRAME>::Index& lambda,
			  const typename RecRingFrame<IFRAME>::Support& supp2,
			  typename RecRingFrame<IFRAME>::Support& supp)
  {
    typedef typename RecRingFrame<IFRAME>::Support Support;
...
//    if (supp1.xmin[0] == -1 && supp1.xmin[1] == -1 && supp1.xmin[2] == -1 && supp2.xmin[3] == -1) return false;
  ...
  inline
  bool intersect_supports(const RecRingFrame<IFRAME>& frame,
			  const typename RecRingFrame<IFRAME>::Index& lambda1,
			  const typename RecRingFrame<IFRAME>::Index& lambda2,
			  typename RecRingFrame<IFRAME>::Support& supp)
  {
    typedef typename RecRingFrame<IFRAME>::Support Support;
