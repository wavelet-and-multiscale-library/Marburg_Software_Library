// implementation for cached_quarklet_slitdomain_problem.h

namespace WaveletTL
{
    template <class PROBLEM>
    CachedQuarkletSlitDomainProblem<PROBLEM>::CachedQuarkletSlitDomainProblem(PROBLEM* P,
    ...
    template <class PROBLEM>
    double
    CachedQuarkletSlitDomainProblem<PROBLEM>::a(const Index& la,
...
//                  cout << "Ergenis: " << r << endl;
    ...
    template <class PROBLEM>
    void
    CachedQuarkletSlitDomainProblem<PROBLEM>::add_ball(const Index& lambda,
				      Vector<double>& w,
				      const int radius,
				      const double factor,
				      const int maxlevel,
...
//                        printf("writing to vector w \n");
    ...
    template <class PROBLEM>
    double
    CachedQuarkletSlitDomainProblem<PROBLEM>::norm_A() const
    {
        if (normA == 0.0)
        {
            //const int offset = 2;
            ...
            cout << "CachedQuarkletSlitDomainProblem()::norm_A() called..." << endl;
#endif
...
//            cout << "cached_slitdomain_problem.norm_A :: last quarklet = " << (problem->frame().last_quarklet(multi_degree(problem->frame().j0())+offsetj,p)) 
...
//            double help;
 //           LanczosIteration(A_Lambda, 1e-6, help, normA, 200, iterations);
    ...
    template <class PROBLEM>
    void
    CachedQuarkletSlitDomainProblem<PROBLEM>::normtest(unsigned int offsetj, unsigned int offsetp) const
    {
        //if (normA == 0.0)
...
//#endif
    ...
    template <class PROBLEM>
    double
    CachedQuarkletSlitDomainProblem<PROBLEM>::norm_Ainv() const
    {
...
#if _WAVELETTL_CACHEDPROBLEM_VERBOSITY >= 1
            cout << "CachedQuarkletSlitDomainProblem()::norm_Ainv() called..." << endl;
...
//            LanczosIteration(A_Lambda, 1e-6, help, normA, 200, iterations);
    ...
    template <class PROBLEM>
    void
    CachedQuarkletSlitDomainProblem<PROBLEM>::set_f(const Function<PROBLEM::space_dimension>* fnew)
    {
        problem->set_f(fnew);
    }
