...
//   discrete_poisson.set_Ainv(1.0/0.146);
  ...
  std::ofstream ofs5("./sd_results2D_46/approx_sol_steep_2D_out.m");
  ...
  for (int i = 0; i < frame.n_p(); i++) {
    ...
    char filename3[128];
    sprintf(filename3, "%s%d%s%d%s%d%s", "./sd_results2D_46/approx2Dsteep_local_on_patch_" , i , "_d" , d ,  "_dT", dT, ".m");
  ...
  for (int i = 0; i < frame.n_p(); i++) {
    ...
    char filename4[50];
    sprintf(filename4, "%s%d%s%d%s%d%s", "./sd_results2D_46/coefficient_plot_2D_patch_" , i , "_d" , d ,  "_dT", dT, ".m");
