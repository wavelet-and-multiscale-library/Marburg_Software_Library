                            ...
                            const unsigned int component = 0) const
        {
            ...
            switch(N) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    return 4*M_PI*M_PI*sin(2*M_PI*p[0]);
...
int main()
{
  ...
  std::set<Index> Lambda;
	...
	if (lambda == basis.last_wavelet(jmax,pmax)) break;
      ...
      setup_stiffness_matrix(cproblem, Lambda, A_Lambda);
      cout << A_Lambda;
...
//  cout << f << endl;
 ...
 InfiniteVector<double, Index> F1_eta, v, w, u, u2, u3; 
 double eta = 0.01;
 cproblem.RHS(1e-6, F1_eta);
 
#if 0 //test of expand/APPLY
  Index lambda(0,3,0,1);
  cout << cproblem.f(lambda) << endl;
  
  
  MySinus testfkt;
  MyFunction<2> funktion;
  MyFunction<3> ableitung;
...
//  cout << u << endl;
  ...
  basis.expand(&ableitung, 1, jmax, pmax, v);
  //v.scale(&cproblem, -1);
  cout << v << endl;
  cout << F1_eta << endl;
  basis.expand(&funktion, 0, jmax, pmax, w, 1);
  //w.scale(&cproblem, 1);
  APPLY(cproblem, w,eta,u, jmax, DKOR, pmax, 1,1);
  APPLY(cproblem, w, eta, u2, jmax, CDD1);
  //u.compress(1e-5);
  //u.scale(&cproblem, -1);
  cout << u << endl;
  cout << u2 << endl;
//  cout << "Integral: " << basis.integrate(&testfkt, lambda) << endl;
  ...
  cproblem.norm_A();
  cout << cproblem.norm_A() << ", " << cproblem.norm_Ainv() << ", " << l2_norm(F1_eta) << endl;
  const double nu1 = cproblem.norm_Ainv() * l2_norm(F1_eta);
  cout << "nu = " << nu1 << endl;
  InfiniteVector<double, Index> solution1_epsilon, rechteseite;
  InfiniteVector<double, double> residuals;
  //residuals(0.89)=0.75;
  cout << residuals << endl;
  
  double epsilon1 = 1e-2;
  CDD2_SOLVE(cproblem, nu1, epsilon1, solution1_epsilon, residuals, jmax, DKOR, pmax, a, b);
  //CDD2_SOLVE(cproblem, nu1, epsilon1, solution1_epsilon, jmax, CDD1);
  cout << solution1_epsilon << endl;
//  cout << w << endl;
//  APPLY(cproblem, solution1_epsilon,eta,u3, jmax, DKOR, pmax, a,b);
//  cout << u3 << endl;
//  cout << v << endl;
//  //cout << solution1_epsilon.size() << endl;
//  cout << "Fertig" << endl;
//  
//  Vector<double> loesung(solution1_epsilon.size());//hier weitermachen@PHK siehe APPLY-Methode kopiren in Vector
  //cout << solution1_epsilon[0] << endl;
  
//  for (unsigned int i = 0; i < solution1_epsilon.size(); i++) {
//	if (solution1_epsilon[i] != 0) {
//	  loesung[i]= solution1_epsilon[0];
//	}
//      }
//  cout << loesung << endl;
//  A_Lambda.apply(solution1_epsilon, rechteseite);
//  cout << rechteseite;
...
//      testvkt[Index(2,1,3)] = 0.0544891;
  ...
  string filenameCoefficients[4] = {"../../Desktop/plotsc++/coefficients_p_0.m", "../../Desktop/plotsc++/coefficients_p_1.m",
  "../../Desktop/plotsc++/coefficients_p_2.m", "../../Desktop/plotsc++/coefficients_p_3.m"};
  ...
  plot_indices2(&basis, solution1_epsilon, jmax, coeff_stream1, p, "jet", false, true, -8);
  coeff_stream1 << "title('coefficients of the test problem ("
 ...
 //SampledMapping<1> sm1(basis.evaluate(w, 8));
  ...
  std::ofstream u_stream1("../../Desktop/plotsc++/plotthis.m");
  sm1.matlab_output(u_stream1);
  u_stream1 << "figure;\nplot(x,y);"
            << "title('mygraph " << basis_type << ", jmax=" << jmax << ", pmax=" << pmax << "');" << endl;
  u_stream1.close();
