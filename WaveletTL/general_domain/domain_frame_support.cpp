...
//    frame.support(lambda, supp1);
  ...
  inline
  bool intersect_supports(const DomainFrame<IFRAME, NPATCHES>& frame,
			  const typename DomainFrame<IFRAME, NPATCHES>::Index& lambda1,
...
//	  intersecting.push_back(*frame.get_quarklet(it));
	...
	if (it == lastquarkletnumber) break;
      ...
      Index last_qua(last_quarklet<IFRAME, NPATCHES>(&frame, j, p));
      int lastquarkletnumber(last_qua.number());
//      Index last_qua(frame.last_quarklet(j,p));
      Index mu = first_quarklet<IFRAME, NPATCHES>(&frame, j, p);
...
//	  intersecting.push_back(*(frame.get_quarklet(it)));
  ...
  void intersecting_quarklets(const DomainFrame<IFRAME, NPATCHES>& frame,
			     ...
			     std::list<int>& intersecting,
                             const typename DomainFrameIndex<IFRAME, NPATCHES>::polynomial_type& p)
  {
//    typedef typename DomainFrame<IFRAME>::Index Index;
    typedef typename DomainFrame<IFRAME, NPATCHES>::Support Support;
