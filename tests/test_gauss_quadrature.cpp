...
int main()
{
  ...
  for (unsigned int N(1); N <= 10; N++)
    {
#if 1
      // set up moments of w(x)=1 on [-1,1] (yields a bit more exact values)
	...
	if (n%2 == 0)
      ...
      Monomial Tk;
      GaussRule GL(moments, Tk, -1.0, 1.0, N);
#else
      // set up moments of w(x)=1 on [0,1]
      Array1D<double> moments(2*N);
      for (unsigned int n(0); n < moments.size(); n++)
	moments[n] = 1./(n+1); // = 1^{n+1}/(n+1)

      Monomial Tk;
      GaussRule GL(moments, Tk, 0.0, 1.0, N);
#endif
      cout << "* Gauss-Legendre rule from generalized moments (N=" << N << "): " << endl;
      Array1D<Point<1> > points;
      Array1D<double> weights;
      GL.get_points(points);
      GL.get_weights(weights);
