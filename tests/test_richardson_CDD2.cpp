...
//       }
  ...
  Array1D<SampledMapping<1> > U = evalObj.evaluate(frame, u_epsilon, true, 13);//expand in primal basis
  cout << "...finished plotting approximate solution" << endl;
  Array1D<SampledMapping<1> > Error = evalObj.evaluate_difference(frame, u_epsilon, exactSolution1D, 13);
  cout << "...finished plotting error" << endl;
  std::ofstream ofs5("approx_sol_rich_1D_out.m");
  matlab_output(ofs5,U);
  ofs5.close();

  std::ofstream ofs6("error_rich_1D_out.m");
  ...
  WaveletTL::plot_indices<Basis1D>(frame.bases()[0]->bases()[0], indices[0], 10, ofs7, "jet", true, -8);



  std::ofstream ofs8("indices_patch_1.m");
  WaveletTL::plot_indices<Basis1D>(frame.bases()[1]->bases()[0], indices[1], 10, ofs8, "jet", true, -8);
#endif
